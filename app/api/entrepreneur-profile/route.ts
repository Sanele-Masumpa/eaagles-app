...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const parsedInvestmentOpportunities: InvestmentOpportunityInput[] = investmentOpportunities ? JSON.parse(investmentOpportunities) : [];

    // Check for invalid investment opportunities
    const validOpportunities = parsedInvestmentOpportunities.filter(opp => 
      opp.title && (opp.amount === undefined || !isNaN(opp.amount))
    );
